...
const state = {
  headerText: 'My MiniBlog',
  memoItemArr: [],
  // memoItemArr: storage.getData(),
  iconArr: ['weather1.png', 'weather2.png', 'weather3.png']
};

const actions = {
  fetchReadMemo(context){
    axios.get('http://seaho5599.dothome.co.kr/page-miniblog/read.php')
    .then(res => {
      // console.log(res)
      // console.log(res.data.result)
      // console.log(res.data.result.length)
      context.commit("READ_MEMO", res.data.result)
  ...
  fetchAddMemo(context, obj) {
    ...
    axios.get(`http://seaho5599.dothome.co.kr/page-miniblog/write.php?user=${addData.user}&title=${addData.title}&date=${addData.date}&icon=${addData.icon}`)
    .then(res => {
  ...
  fetchDeleteMemo({commit}, obj) {
    ...
    // 아래를 실행한다.
    console.log('삭제', obj)
    axios.get(`http://seaho5599.dothome.co.kr/page-miniblog/delete.php?id=${obj.id}`)
    .then(res => {
      console.log('서버 회신', res.data)
  ...
  fetchUpdateMemo({
    ...
    axios.get(`http://seaho5599.dothome.co.kr/page-miniblog/update.php?id=${obj.item.id}&complete=${complete}`)
    .then(res => {
  ...
  fetchClearMemo({
    ...
    // 정상적으로 처리되었다면
    // 아래를 실행한다.
    axios.get("http://seaho5599.dothome.co.kr/page-miniblog/delete.php?id=all")
    .then(res => {
      console.log(res.data);
      commit("CLEAR_MEMO");
    })
...
const mutations = {
  READ_MEMO(state, payload){
    // 속성중 complete 를 true와 false로 교체
  ...
  ADD_MEMO(state, payload) {
    ...
    axios.get(`http://seaho5599.dothome.co.kr/page-miniblog/get.php?user=${payload.user}`)
    .then(res=>{
      // console.log('입력완료', res)
